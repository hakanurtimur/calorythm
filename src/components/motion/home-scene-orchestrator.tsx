...
export function HomeSceneOrchestrator({ children }: HomeSceneOrchestratorProps) {
  ...
  useEffect(() => {
    ...
    async function initialise() {
        ...
        media.add(
          ...
          (context) => {
            const conditions = context.conditions as
              | { isDesktop: boolean; isMobile: boolean; reduceMotion: boolean }
              | undefined;
            const profile = readMotionProfile();

            if (conditions?.reduceMotion || !profile.animate) {
      ...
      handleConnectionChange = configure;
      configure();
    ...
    return () => {
      active = false;
      if (handleConnectionChange) connection?.removeEventListener("change", handleConnectionChange);
      media?.revert();
    };
  }, []);
